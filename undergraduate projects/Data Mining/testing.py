...
def trainModelAndGetHistory(X_train, y_train, epochs=100, batch_size=32):
...
def plotLossCurves(histories, titles):
    plt.figure(figsize=(16, 10))

    for history, title in zip(histories, titles):
        plt.plot(history.history['loss'], label=title + ' loss')
...
def trainAndPlot(datasets, titles):
    ...
    for ds, title, ax in zip(datasets, titles, axs.flatten()):
        ...
        model, history = trainModelAndGetHistory(X_train, y_train)
        ...
        # Plot loss curves
    plotLossCurves(histories, titles)
